...
public class UPCEEncoder extends EANEncoder {
	...
	public BarSet[] encode(String texto) throws InvalidAtributeException {
		...
		try{		
			...
			for (int i = 0; i < 6; i++) {
				...
				if( par.get(i) ){			
					result[i + 2] = CODES[atual].reverse();
				} else {
					result[i + 2] = CODES[atual].xorTrue();
				}
			}
			//barras centrais que dividem o codigo
			result[8] = RIGTH_GUARD_EXT;
            result[9] = QUIET_ZONE;
	...
	public static String convertUPCEtoUPCA(String upce) throws InvalidAtributeException{
	...
	public String computeCheckSum(String text) throws InvalidAtributeException {
        ...
        return UPCAEncoder.getInstance().computeCheckSum(upca);
	}
